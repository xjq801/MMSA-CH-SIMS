...
| 45 T0历史收益可学习性诊断 | AUTHORIZED_PENDING_CREATION | 未创建 | `SC-20260806-01`、v1.24、Task45研究方案/预注册/矩阵与`.light/carm-v124-*`合同 | 仅原train的FIT→DIAG_CONFIRM；两条primary必须同时通过；旧DEV、TRAIN_ROUTER_CONFIRM、formal test零事件；不训练router、不执行负迁移/P5 | 创建后`HANDOFF_45.md`；后续S43 |
