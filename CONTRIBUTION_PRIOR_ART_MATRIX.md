...
|---|---|---|---|---|---|
| C1：严格T0公众诱发情绪分布预测 | Video2Reaction；NEmo+；CSMV/MSA-CRVI；MVIndEmo | 从内容预测完整受众诱发情绪分布；使用多人/评论反应 | 只允许声称HUMAN_GOLD、target评论推理不可见、video/post-group与topic held-out、未来信息物理隔离和系统审计；不再允许任务首创 | Video2Reaction论文VLM/LDL设置的公平适配或不可执行审计；内容先验；冻结视频特征+MLP；NEmo+分布baseline；VC-CSA仅作NON_T0诊断 | 若主要差异只剩数据集/划分或Video2Reaction适配基线未处理，C1只能作为协议/证据贡献；目标评论或未来互动进入输入则C1失败 |
...
- 禁止：首次提出公众诱发情绪、首次做情绪分布预测、首次从视频内容预测受众反应分布、首次video-to-reaction-distribution、首次将特权信息用于情感识别、首次做检索增强缺失模态情绪识别、首次做缺失模态拒绝。
