...
> 数据：CSMV HUMAN_GOLD，`group_by_video_v1`；除明确legacy失败行外  
> 主指标：Jensen–Shannon divergence；test尚未查看
...
| 原48维+CatBoost | LEGACY | CUC legacy 48维 | FAILED_DATA_MISMATCH_NO_FROZEN_SPLIT_T0_INELIGIBLE | 无 | 不复用旧数值 |
| 原48维+HGB | LEGACY | CUC legacy 48维 | FAILED_DATA_MISMATCH_NO_FROZEN_SPLIT_T0_INELIGIBLE | 无 | 不复用旧数值 |
| 原48维+LightGBM | LEGACY | CUC legacy 48维 | FAILED_DATA_MISMATCH_NO_FROZEN_SPLIT_T0_INELIGIBLE | 无 | 不复用旧数值 |
...
正式表格数值须在可用GPU环境中完成预注册dev选择、单种子完整run与test一次性评测后另行填入；本表不包含旧论文数字或CPU smoke数值。
