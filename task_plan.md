...
### 阶段7：开工准备包审计与补齐
- [ ] 按总纲八类准备项建立“已完成/部分完成/缺失”证据表
- [ ] 跳过已达标项，仅补齐目录与版本、环境、数据存储、实验纪律、安全合规、时间资源、文献写作缺口
- [ ] 执行最小导入、密钥/大文件/Git忽略、配置重跑能力等验收
- [ ] 形成开工准备验收报告，并把不可代替用户完成的外部动作明确隔离
- [ ] 更新总纲/项目记忆中的实际准备状态，通知任务10读取交付物
- **状态：** in_progress
