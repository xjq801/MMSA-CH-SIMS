...
3. B07/B08：任务最近前作，但必须严格标记输入边界与数据许可；不能把非T0方法伪装为公平content-only。
4. H1阶段再启用B09/B10；H2阶段再启用B11—B14；H3正式实验再从B15—B17选近期强基线。

## 4. 当前阻塞

- G1仍因第二人工多模态公开主集未冻结而`BLOCKED`；baseline清单完成不代表可以训练。
- 正式环境缺`faiss`，RAMER原样复现暂为`BLOCKED_M1`；现在不安装。
