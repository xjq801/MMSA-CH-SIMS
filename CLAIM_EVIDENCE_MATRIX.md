...
| C3 | train-only反应记忆与收益感知可靠性路由优于无检索和随机检索，并减少负迁移 | 无/随机/稀疏/稠密/可学习检索、cross-fitting效用标签、固定融合/简单拒绝对照、错误邻居与risk-coverage | H2、E2/E4/E7、任务40/50 | TO_VERIFY | 不得宣称检索增益或可靠性改善 |
...
## 2026-07-23 v1.17计划效应门（不升级claim状态）

| 层级 | 预注册判据 | 允许动作 |
|---|---|---|
| 开发趋势 | 相对最终最强公平基线JSD改善至少3%，方向稳定，ECE恶化不超过0.005 | 允许进入下一开发任务，不得写为论文结论 |
| 论文最低 | JSD改善至少5%，按数据集原生内容单元paired bootstrap 95% CI优于0，多重校正后成立 | 可支持对应准确性claim |
| 强结果 | JSD改善至少8%，NLL/EMD方向一致且校准改善 | 可支持较强方法claim |
| 可靠性Pareto | JSD绝对非劣界`+0.003`内，AURC改善至少10%、负迁移率下降至少20%、ECE不恶化 | 可支持对应可靠性claim |

以上是任务30/40/50的事前计划门，不是当前结果；当前C1—C4有效性状态保持`TO_VERIFY`。
