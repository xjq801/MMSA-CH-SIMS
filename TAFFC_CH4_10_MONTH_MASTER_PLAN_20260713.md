...
> **Comment-privileged forecasting of public-induced emotion distributions with a calibrated audience-response memory under incomplete multimodal evidence.**
...
### 4.2 主公开 benchmark B：iNews（首选，设可用性门）

- 2899个多模态新闻帖、291位参与者、平均5.18个标注/帖；
- 含VAD、离散情绪和模态重要性，可形成内容诱发的人群反应分布；
- 与微视频平台存在域差异，但这正好用于检验跨场景泛化；
- 第一月必须验证数据许可、媒体内容可得性、标签到统一情绪空间的映射。

**切换规则**：2026-08-01前若iNews无法合法获得或不能形成可复现输入，立即切换NEmo+；第二主测试集必须优先选择已有多人类标注的数据。
...
| ID | 假设 | 主检验 | 反证/止损条件 |
|---|---|---|---|
| H1 | 评论特权教师能改善内容学生对未来群体反应分布的预测 | CSMV与第二公开集的JS/NLL | 对两主集最强content-only基线均无稳定改善，或提升只来自随机划分 |
| H2 | train-only反应记忆在相关域中有效，可靠性拒绝可减少OOD负迁移 | learned retrieval vs no/random/BM25/CLIP-kNN；AURC/ECE | 随机检索同样好，或跨话题/平台下校准和分布误差更差 |
| H3 | 模态质量/缺失感知可使单一模型在完整与缺失输入下平稳退化 | 全模态、单模态、随机/自然缺失 | 不优于简单late fusion、zero-fill或近期缺失模态基线 |
| H4（增强） | 配对模态条件能帮助预测加入图像后受众反应分布的变化 | NEmo+上的 `p_TI-p_T`、`p_TI-p_I` | 无法超过独立预测两个分布后相减的简单基线 |

主成功门：在两个公开主数据集上，视频级paired bootstrap 95%置信区间支持我方在主要分布指标上优于最强公平基线，且校准不恶化；或形成明确的“分布误差不劣、OOD校准/选择性风险显著更优”的Pareto优势。单个随机split高分不算成功。
...
7. RAMER式检索适配及至少两个当前可运行的缺失模态强基线；
...
- 统计：至少5个随机种子；视频级paired bootstrap 95%CI；配对检验与多重比较校正；
- 不把seed或fold数量当独立样本量。
...
退出门G4：H1/H2至少有一条在两个主集成立，另一条有一致趋势或明确边界；随机split不是唯一优势；校准不恶化。否则降低目标、重构问题或转数据/评测贡献，不再堆模块。
...
- iNews/NEmo+↔CSMV之间跨数据/跨场景验证；MVIndEmo仅补充银标迁移实验；
