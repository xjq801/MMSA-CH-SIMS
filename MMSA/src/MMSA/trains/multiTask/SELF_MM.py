...
class SELF_MM():
    def __init__(self, args):
        ...
        self.args = args
        self.prototype_loss_weight = float(args.get('prototype_loss_weight', 0.0))
        self.aux_5class_loss_weight = float(args.get('aux_5class_loss_weight', 0.0))
        self.prototype_diversity_loss_weight = float(args.get('prototype_diversity_loss_weight', 0.0))
        self.aux_5class_weights = None
        self.gate_regularization_weight = float(args.get('gate_regularization_weight', 0.0))
        self.gate_entropy_floor = float(args.get('gate_entropy_floor', 0.8))
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        if self.aux_5class_loss_weight > 0.0:
            targets = self.labels_to_5classes(self.label_map['fusion'])
            counts = torch.bincount(targets, minlength=5).float()
            self.aux_5class_weights = targets.numel() / (5.0 * counts.clamp_min(1.0))
            logger.info("Aux-5 class counts: %s; weights: %s", counts.tolist(), self.aux_5class_weights.tolist())
        ...
        while True: 
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    loss += self.prototype_guidance_loss(outputs, self.label_map['fusion'][indexes])
                    loss += self.prototype_diversity_loss(outputs)
                    loss += self.aux_5class_loss(outputs, self.label_map['fusion'][indexes])
                    loss += self.gate_regularization_loss(outputs)
                    # backward
                    loss.backward()
    ...
    def prototype_guidance_loss(self, outputs, y_true):
        if self.prototype_loss_weight <= 0.0 or 'PrototypeScore' not in outputs:
            return 0.0
        target = torch.clamp(y_true.view(-1) / float(self.args.H), min=-1.0, max=1.0)
        proto_score = outputs['PrototypeScore'].view(-1)
        strength = 0.5 + 0.5 * torch.abs(target)
        loss = torch.mean(strength * torch.abs(proto_score - target))
        return self.prototype_loss_weight * loss

    def prototype_diversity_loss(self, outputs):
        if self.prototype_diversity_loss_weight <= 0.0 or 'PrototypeDiversity' not in outputs:
            return 0.0
        return self.prototype_diversity_loss_weight * outputs['PrototypeDiversity']

    def gate_regularization_loss(self, outputs):
        if self.gate_regularization_weight <= 0.0 or 'GateWeights' not in outputs:
            return 0.0
        weights = outputs['GateWeights'].clamp_min(1e-8)
        entropy = -torch.sum(weights * torch.log(weights), dim=-1)
        entropy_floor = self.gate_entropy_floor * math.log(weights.size(-1))
        return self.gate_regularization_weight * torch.relu(entropy_floor - entropy).mean()

    def labels_to_5classes(self, labels):
        boundaries = labels.new_tensor([-0.7, -0.1, 0.1, 0.7])
        return torch.bucketize(labels.view(-1), boundaries, right=False)

    def aux_5class_loss(self, outputs, y_true):
        if self.aux_5class_loss_weight <= 0.0 or 'Aux5Logits' not in outputs:
            return 0.0
        targets = self.labels_to_5classes(y_true)
        return self.aux_5class_loss_weight * F.cross_entropy(
            outputs['Aux5Logits'], targets, weight=self.aux_5class_weights
        )
