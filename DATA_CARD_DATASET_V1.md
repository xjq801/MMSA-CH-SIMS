...
> 状态：`FORMAL_PROTOCOL_DATA_G2_PASS_ASSET_RISK_ACCEPTED`；LAI-GAI第二主集已`FROZEN_00_APPROVED`  
> 版本日期：2026-07-17  
> 权威机器清单：`data/manifests/dataset-v1.manifest.json`

## 数据集摘要

dataset-v1是任务20内部研究使用的正式协议输入合同。第一人工主集CSMV将107,267条人工评论反应按8,210个内部视频聚合，并通过官方URL元数据归并为8,008个源视频族；第二人工跨域图像主集LAI-GAI含847图与63,682个人工反应。G1与`G2_PROTOCOL_DATA`均已通过，CSMV lineage/split修复已由00接受，当前19项公共核心复现零漂移。I3D资产级许可、稳定官方revision及权利方包身份/fixity仍未获维护者确认，按`SC-20260717-01`记为`DEFERRED_ACCEPTED_RISK`，不被改写为已解决。

## 预期用途

- 审计公众诱发受众情感分布的标签工程、split和泄漏边界。
- 作为任务20内部研究中T0内容侧预测的版本化输入合同。
- 复核标签不确定性、标签冲突与数据缺陷。

当前允许按冻结协议进行内部正式模型比较并形成候选论文证据；在资产风险未关闭前，不得公开分发I3D特征、把维护者许可/官方包身份写成已确认，或把相关工件作为可再分发benchmark发布。仍禁止银标与人工test合并、个体用户画像、内容审核自动裁决、政治定向、重新识别或平台用户追踪。
...
split在可观察ID/source-group范围内零Critical、第二主集已冻结且当前隔离复现已关闭。00已按`SC-20260717-01`批准`G2_PROTOCOL_DATA=PASS_WITH_LIMITATIONS`并接受资产风险延期，当前`formal_split=true`；该状态只授权内部研究，不授予I3D再分发权。
...
- CSMV正式输入已收敛到I3D。其本地文件树、体量、逐文件hash、`float32[T,1024]` schema及8210/8210覆盖已闭合；资产级许可、稳定官方revision及权利方包身份/fixity为`DEFERRED_ACCEPTED_RISK`。允许任务20内部训练与评估，但禁止再分发I3D；论文必须披露不确定性。若权利方否认使用许可或身份/hash/覆盖发生漂移，相关运行立即停止并失去正式证据资格。
