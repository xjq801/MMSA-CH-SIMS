...
1. 研究与阶段需求以`TAFFC_CH4_10_MONTH_MASTER_PLAN_20260713.md`当前版本为唯一总纲；任务10以总纲第17节任务10为准。
   - 本项目从总纲v1.15起只执行T-AFFC的CARM群体情绪预测路线；当前执行版本为v1.24（Task30/40负结果不变；Task45只做原train内T0收益概率/幅度可学习性诊断，封存旧DEV、TRAIN_ROUTER_CONFIRM与formal test；Task46/50未创建；保留Video2Reaction双轨强基线，不恢复已撤回v1.17数值门）。
