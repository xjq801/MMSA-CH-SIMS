...
1. 只读核验Git、HANDOFF_20 manifest、Task20环境/配置/预测schema和任务30适用字段，产出delta审计。
2. 冻结Task30独立环境、teacher可见字段、train/dev开发合同和H1成功/失败/无结论规则。
3. 先写负测：test/dev评论不可达、错配teacher可识别、分布归一化、非有限值、数据集特定head与缺字段fail-closed；再实现最简单teacher/student链。
