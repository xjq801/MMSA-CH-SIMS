# 群体情绪预测双论文十个月总纲（T-AFFC主线 + IJCV条件专刊线）

> 版本：v1.14  
> 冻结日期：2026-07-16  
> 执行周期：2026-07-13—2027-05-12  
> 首要目标：形成两篇实质独立、证据可审计的候选论文：2026-12-15前条件冲刺IJCV专刊的主观视觉表征论文；2027-05-12前完成T-AFFC的CARM群体情绪预测论文。任一路线未过硬门即止损，不以一稿多投或降低证据标准换取双投稿。  
...
- 00冻结复审已放行G1；`REVIEW-00-CSMV-LINEAGE-G2-20260715`已接受CSMV命名空间纠正与同源split修复。`REVIEW-00-CSMV-FEATURE-PREFLIGHT-G2-20260715`又独立确认19项公共核心复现当前零漂移，正式关闭`REPRODUCIBILITY_STALE`子阻塞；G2现唯一收敛为CSMV正式输入资产的许可、revision/fixity、机器manifest与8210覆盖阻塞，状态为`BLOCKED_CSMV_INPUT_ASSET_LICENSE_FIXITY_AND_COVERAGE`。全局`formal_split=false`，不允许创建任务20或训练。
...
- 许可未知的公开资产可以先下载审计，但只标`QUARANTINE_ACQUIRED`；法律许可不能由项目自行扩大，未闭合前不得正式训练、建索引、发布、再分发或获得G门信用。
...
- 当前CSMV可并行寻找官方副本或可信镜像并隔离取得候选特征族，但G2、全局`formal_split=false`和任务20禁令不因预取改变。
...
- CSMV正式内容输入收敛为一个经00准入的视觉特征族，优先I3D；评论仅作人工标签来源、训练期特权监督或train-only反应记忆，不得进入T0学生推理输入。
...
### 0.6 IJCV专刊条件双路线（v1.14，`SC-20260716-02`）

- IJCV官方专刊“Social, Emotional, and Cognitive Visual Intelligence”与本项目的主观视觉情绪分布、观察者差异、概率/不确定性和跨域泛化高度匹配，官方截稿为2026-12-15；详细依据与方案见`IJCV_TAFFC_DUAL_TRACK_FEASIBILITY_AND_PLAN_20260716.md`。
- 裁定为`CONDITIONAL_GO_TWO_DISTINCT_PAPERS`，不是同稿双投：T-AFFC保留评论特权监督、train-only受众反应记忆与可靠性路由；IJCV改为响应分布几何驱动的视觉表征、主观分歧建模和跨域几何保持。
- IJCV不等待CSMV维护者回复。其J0门要求至少两个像素级人工主观分布图像集合法、固定、可复现；CSMV仅作G2通过后的可选视频外验。CSMV G2、`formal_split=false`与任务20禁令不因IJCV范围变更而放宽。
- IJCV条件任务25/65只有在J0/J2分别通过后创建；原任务20—60保持T-AFFC链。两条链的主问题、方法、主表、claim-evidence、文稿与投稿包物理隔离，正式投稿时如实披露相关稿。
- IJCV路线的J0/J1/J2与十项投稿Go标准以第18节为准；双路线时间安排以第18.8节覆盖本节原单线月份顺序，但T-AFFC各G门和质量要求不降低。
...
IJCV论文不得复用C1—C3作为主要贡献；其独立贡献合同与JH1—JH3见第18节。
...
本节保留T-AFFC原始工作包定义；v1.14起，两条论文线的实际日历与任务并行边界以第18.8节为准。IJCV先在2026-12-15前完成条件冲刺，T-AFFC方法开发顺延但最终目标日期仍为2027-05-12。
...
- IJCV专刊：<https://link.springer.com/collections/fdhgfjgaae>
- IJCV范围与文章类型：<https://link.springer.com/journal/11263/aims-and-scope>
- IJCV投稿与原创性规则：<https://link.springer.com/journal/11263/submission-guidelines>
- Progressive Circular Learning（CVPR 2021）：<https://openaccess.thecvf.com/content/CVPR2021/html/Yang_A_Circular-Structured_Representation_for_Visual_Emotion_Distribution_Learning_CVPR_2021_paper.html>
- SAMNet：<https://arxiv.org/abs/2207.11875>
- MFRN（AAAI 2025）：<https://ojs.aaai.org/index.php/AAAI/article/view/32965>

## 17. Codex任务树详细执行规格

> 规格版本：v1.1  
> 并入日期：2026-07-14  
> 权威性：本节是总纲的一部分，负责规定00—65各Codex任务的启动条件、执行步骤、质量水平、退出门和交接要求。  
...
00-总控与决策（持续存在）
  └─ 10-M1–M2 数据与协议（已创建）
       ├─ J0通过后创建 25-IJCV视觉表征与实验
       │    └─ J2通过后创建 65-IJCV专刊写作与投稿
       └─ G1 + G2通过后创建 20-M3 基线与统一评测（T-AFFC）
            └─ G3通过后创建 30-M4 评论教师与内容学生（T-AFFC）
                 └─ H1开发门通过后创建 40-M5 反应记忆与可靠性检索（T-AFFC）
                      └─ CARM-v1冻结后创建 50-M6–M8 正式实验与结果冻结（T-AFFC）
...
任务10、20、25、30、40至少达到L2的各自阶段门；任务50必须达到L3结果冻结；任务60与65必须分别达到各自投稿的L3冻结。
...
- C1—C3贡献边界；
- H1—H4假设；
- G1—G6、J0—J2及T-AFFC/IJCV各自投稿Go标准；
- Codex项目、任务与算力分工。
...
- 各任务创建提示与交接确认；
- 最终`TAFFC_GO_NO_GO.md`与条件性的`IJCV_GO_NO_GO.md`。
...
- T0泄漏测试全部通过；
- CSMV与第二人工跨域图像集均已按各自角色合法、固定、可追溯地复现。
...
| 10/00 | 25 | 至少两个像素人工分布集、J0数据/新颖性门、独立IJCV合同 | 可不等待CSMV G2；但不得与20同时修改共享评测核心 |
| 25 | 65 | IJCV results-freeze、J2、claim-evidence与独立复现包 | 写作骨架可并行，主结果不得提前定稿 |
| 65 | 00 | IJCV submission-ready包、重叠审计与十项Go证据 | 由00执行IJCV最终Go/No-Go |
...
4. 先在任务10/00内完成IJCV第二像素人工集与近邻审计；J0通过后才创建任务25，不等待CSMV维护者回复。
5. 任务10通过G1/G2后，回到00任务审核并创建任务20；任务20不得因IJCV分支绕过G2。
6. 任务25与任务20不得同时修改共享评测核心；IJCV结果冻结后再以交接方式复用通用组件。
7. 其余任务严格按本规格的启动条件逐个创建。
...
## 18. IJCV—T-AFFC条件双论文路线（v1.14新增）

> 完整执行合同：`IJCV_TAFFC_DUAL_TRACK_FEASIBILITY_AND_PLAN_20260716.md`。本节为总纲内权威摘要；若本节与第8节原单线日历冲突，以本节为准。数据与研究诚信硬门仍取两处更严格者。

### 18.1 适配与可行性裁定

- IJCV专刊官方范围直接包含主观视觉理解、情感、概率/不确定性、观察者差异、跨域泛化与人本视觉评测，因此科学对象强匹配。
- 当前冻结I3D + 普通分布头 + CARM的组合不具备足够的IJCV视觉方法性；视觉情绪分布学习已有PC Loss、SAMNet、MFRN等直接前作。
- 最终裁定为`CONDITIONAL_GO_TWO_DISTINCT_PAPERS`：有希望，但不是当前方案原样可投，也不提供录用概率承诺。
- IJCV在2026-12-15前条件冲刺；T-AFFC继续到2027-05-12。任何门失败都停止对应路线，不把一篇稿件同时投往两刊。

### 18.2 两篇论文不可混淆的核心合同

| 维度 | IJCV候选论文 | T-AFFC候选论文 |
|---|---|---|
| 主问题 | 视觉表征能否保持人工主观情绪响应分布的几何与分歧结构 | 评论特权监督与train-only受众记忆能否改善T0未来群体反应预测及选择性可靠性 |
| 方法主轴 | 响应几何对齐、分歧/不确定性建模、跨域几何保持 | teacher—student蒸馏、反应记忆检索、可靠性路由 |
| 核心输入 | 原始图像像素；人工响应分布仅作监督 | CSMV冻结视觉特征；评论只作训练期特权监督/标签/历史记忆 |
| 主数据 | LAI-GAI + 至少一个像素可得人工分布图像集 | CSMV + LAI-GAI；CUC压力测试 |
| 主结果 | JH1—JH3、视觉强基线、分布几何、分歧/跨域 | H1—H4、检索负对照、OOD、AURC/校准 |
| 明确排除 | 评论teacher、CARM memory/router、舆情应用 | 把IJCV几何模块改名后作为第二主创新 |

通用加载器、泄漏测试和基础指标代码可以共享；主假设、核心方法、主结果表、关键图、claim-evidence与正文不得共享为两稿的共同贡献。两稿送审期重叠时，按期刊要求披露相关稿及差异。

### 18.3 IJCV最小方法核

1. **响应几何对齐**：在同一数据集内，用人工分布之间的JS或Wasserstein距离定义目标几何，使视觉嵌入保持成对距离或局部排序，同时用proper distribution loss学习逐样本分布。
2. **分歧与不确定性**：基于逐人响应/计数拟合均值与浓度或预测区间；观察者分歧与模型/域外不确定性分别评测，不把预测熵直接当作全部不确定性。
3. **跨域保持**：标签空间不一致时采用共享视觉编码器和数据集特定head，只检验域内几何保持、线性探测或预注册迁移estimand，不事后强制统一类别。

最小核不得扩张为生成模型、视觉解释LLM、额外大规模标注和多任务模块堆叠。第3项若不能在J1前定义公平estimand，则只作外部有效性实验。

### 18.4 IJCV数据J0门

IJCV主线不等待CSMV Issue #5。2026-08-12前必须从Flickr_LDL、Twitter_LDL、Emotion6、OASIS、NEmo+等候选中，再冻结至少一个像素可得的人工情绪分布集，与LAI-GAI共同组成两个主集。每个主集必须闭合：研究使用许可、固定revision、文件树/hash、人工响应来源、独立样本单位、近重复/group-safe split、标签空间与响应计数。

CSMV只有在G2通过后才可作为可选视频外验；没有CSMV不阻塞IJCV。第二像素人工集或新颖性矩阵未过门，则裁定`NO_GO_IJCV_SPECIAL_ISSUE_DATA_OR_NOVELTY`，回到T-AFFC或以后再考虑regular IJCV。

### 18.5 IJCV可证伪假设

| ID | 假设 | 主检验 | 止损条件 |
|---|---|---|---|
| JH1 | 响应几何对齐改善视觉情绪分布预测 | 至少两个图像人工集的JSD/EMD/NLL，按图像paired bootstrap与5种子 | 两集均无达到预注册SESOI的稳定提升，或仅随机split有效 |
| JH2 | 分歧建模改善区间/选择性可靠性且分布精度非劣 | 响应分歧、覆盖、NLL/Brier、AURC；softmax/Dirichlet/ensemble公平比较 | 分歧/覆盖/AURC均无改善，或JSD恶化超过非劣界 |
| JH3 | 几何保持改善跨域迁移而不依赖事后标签映射 | source-only、普通fine-tune、线性探测、预注册域适配对照 | 只在事后映射下有效，或不优于普通fine-tune/强foundation baseline |

种子不是独立样本量；统计按图像/视频等内容单元执行。正式test前冻结主指标、SESOI、非劣界、种子、调参预算与多重比较族。

### 18.6 IJCV基线与单变量矩阵

- 必跑：数据先验；ResNet/ViT/CLIP/SigLIP + KL/JSD/softmax/Dirichlet头；PC Loss；SAMNet；MFRN；等预算metric/supervised contrastive；temperature scaling/deep ensemble；source-only/fine-tune/一个强域适配基线。
- 单变量实验：J0数据审计、J1强基线、J2增加响应几何、J3替换几何定义、J4增加分歧head、J5不确定性等预算对照、J6跨域、J7逐组件消融、J8噪声/低响应数/域移、J9效率。
- 全部基线使用同一split、图像预处理、trial预算、early stopping与test脚本；无法适用必须书面说明，不得静默删除。

### 18.7 三个IJCV阶段门

| 门 | 最迟日期 | 通过条件 | 失败动作 |
|---|---|---|---|
| J0 数据/新颖性 | 2026-08-12 | 两个像素人工集准入；近邻差异矩阵；JH/指标/SESOI预注册 | 停止专刊冲刺 |
| J1 基线/可实现性 | 2026-09-12 | 两集强基线可信复现；统一评测冻结；计算预算可承受 | 降级为T-AFFC共享基建，不开发IJCV方法 |
| J2 证据/写作 | 2026-11-15 | JH1成立，JH2/JH3至少一条有独立证据；完整单变量消融和统计 | 不创建任务65，不赶稿 |

### 18.8 双线日历

| 时间 | 主执行 |
|---|---|
| 07-16—08-12 | IJCV J0：第二像素人工集、近邻与预注册；任务10仅处理影响发表且不依赖维护者回复的事项 |
| 08-13—09-12 | 条件任务25建立像素管线、复现PC/SAMNet/MFRN与强foundation基线；J1 |
| 09-13—10-15 | 响应几何最小核与开发集止损；无最小效应即停止 |
| 10-16—11-15 | 五种子J2—J9、统计、分歧与跨域；J2 |
| 11-16—12-10 | 条件任务65完成IJCV稿件、图表、复现包、模拟审稿和两稿重叠审计 |
| 12-11—12-15 | 仅通过十项Go标准时投稿IJCV专刊 |
| 12-16—2027-01-31 | T-AFFC任务20/30：统一基线与H1 |
| 02-01—02-28 | T-AFFC任务40：反应记忆与H2 |
| 03-01—03-31 | T-AFFC任务50：正式实验、OOD、结果冻结 |
| 04-01—04-30 | T-AFFC任务60：写作、图表、复现与模拟审稿 |
| 05-01—05-12 | 仅通过九项Go标准时投稿T-AFFC |

### 18.9 新增条件任务规格

#### 任务25：IJCV视觉表征与实验

**启动**：J0书面通过；两个像素主集冻结；IJCV假设/指标/SESOI/调参预算预注册。无需等待CSMV G2。  
**步骤**：读取J0交接；锁定像素管线；先复现J1强基线；测试先行实现J2—J9；仅在dev作方法选择；完成5种子与内容单元统计；形成IJCV独立claim-evidence；冻结结果并请求J2。  
**产出**：`HANDOFF_25.md`、`IJCV_EXPERIMENT_PROTOCOL.md`、`ijcv-evaluation-kit`、`ijcv-baseline-table`、`ijcv-results-freeze`、统计报告、失败/止损报告。  
**达标**：至少L2；J2通过才可创建65。  
**禁止**：读取评论正文、实现CARM、使用未准入CSMV、根据test加模块、与任务20并行修改共享评测核心。

#### 任务65：IJCV专刊写作与投稿

**启动**：J2通过；IJCV结果与claim-evidence冻结；00批准写作。  
**步骤**：按“视觉问题—技术洞察—方法—证据—边界”写作；核对官方专刊当日状态；完成出版图、复现包、引用/诚信/许可审计；与T-AFFC材料做主问题/方法/主表/正文重叠审计；模拟IJCV AE与审稿人；逐条执行十项Go标准。  
**产出**：`IJCV_GO_NO_GO.md`、独立manuscript、图表、匿名复现包、cover letter、相关稿披露说明和`HANDOFF_65.md`。  
**达标**：L3且无未解决Critical。  
**禁止**：复用CARM作为主创新、隐藏相关稿、同稿双投、因截止日跳过统计/许可/复现门。

### 18.10 IJCV投稿十项Go标准

1. 专刊在提交当日仍开放；2. 两个像素人工分布主集合规固定；3. JH1成立且JH2/JH3至少一条有独立证据；4. 相对PC Loss、SAMNet、MFRN及强foundation baseline有公平统计增量；5. 单变量消融可归因；6. 五种子、内容单元CI、效应量、校准/选择性、效率与失败案例完整；7. 标签空间、观察者代表性、AI生成图像域与许可边界诚实；8. 匿名复现包能重跑关键结果；9. 与T-AFFC稿无主问题/方法/主表/文本重复发表Critical；10. 模拟审稿无未解决的视觉方法性、近邻撞车、小数据、事后标签映射或统计单位Critical。
