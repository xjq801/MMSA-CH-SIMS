...
| task20-vccsa-author-local-gpu-smoke-20260718 | 2026-07-18 | `configs/task20/vccsa-author-original-v1.json` + author fork `3e8c426` | author comments + restricted local I3D read-only | author comment split smoke：train=8/dev=4/test=0 | AUTHOR_ORIGINAL_SETTING_NON_T0 | batch=1 / 1 epoch / RTX 3070 Ti / 146.05439M params | 3407 | author opinion/emotion dev metrics（仅可执行性） | test访问、OOM或入口失败即停 | VALIDATION_ONLY | 训练段约4秒并完成dev；全量因约52天本地估算及A30不可达未启动；`TASK20_BASELINE_EXECUTION_AUDIT.md` |
