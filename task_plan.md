...
- [x] 运行一致性、工作日志与综合准备检查，提交并同步GitHub
- **状态：** completed

### 阶段22：IJCV项目迁出与T-AFFC单路线恢复（2026-07-16）

- [x] 确认独立IJCV项目已保有未删减总纲v1.14、上下文交接、独立分支和启动任务。
- [x] 将当前项目总纲升级为v1.15，恢复T-AFFC为唯一活动路线。
- [x] 从当前任务树和日历移除J0—J2、JH1—JH3、任务25/65及IJCV投稿门。
...
- [x] 创建任务20并绑定Git提交`f869732`；任务20只允许内部训练、train-only索引和冻结评估，不得再分发I3D或虚构权利方确认。
- **状态：** completed_task20_thread_019f6e2e_f781_7270_bb45_af8272ff5a5c_started

### 阶段24：Video2Reaction直接前作中修与SSOT v1.19（2026-07-24）

- [x] 刷新共享主仓库、origin/main、Task20线程和外部Word总纲现实状态
- [x] 核验arXiv、DataMFM官方页面、作者出版页与ECCV/ECVA公开入口
- [x] 冻结新论文定位、claim blacklist、closest-prior关系和三类拒稿预演
- [x] 回扫总纲、H1—H4、E0—E9、任务30—60接口及claim/风险/基线台账
- [x] 建立Word派生文档回填合同，不把v1.14外部Word恢复为SSOT
- [x] 运行工作日志、准备检查、文本一致性、handoff合同与Git门禁
- [x] 提交并推送主线，通知Task20刷新
- **状态：** completed
- **边界：** 不改变G1—G3，不修改Task20实验核心，不恢复v1.17的3%/5%/8%硬阈值，不创建Task30。

#### 阶段24错误记录

| 错误 | 尝试次数 | 解决方案 |
|---|---:|---|
| 项目定位校验器把总纲claim blacklist中的禁止句误判为正向主张 | 1 | 收紧为正向“我们/本文”句式；blacklist自身改由required contract校验 |
